        ...
        VStack{
            ...
            if !inCalibrationConfirmation {
                if calibrationState == CalibrationState.buttons {
                    CalibrationButtonsView(state: $state, calibrationState: $calibrationState, selectState: $selectState, queue: $queue, value: $value, showConfirmationScreen: $showConfirmationScreen, nextStateId: $nextStateId, prevState: $prevState, playSound: $playSound, lookUpSens: $lookUpSens, lookDownSens: $lookDownSens, lookLeftSens: $lookLeftSens, lookRightSens: $lookRightSens, defaultUp: $defaultUp, defaultDown: $defaultDown, defaultLeft: $defaultLeft, defaultRight: $defaultRight)
                } else if calibrationState == CalibrationState.up || calibrationState == CalibrationState.down || calibrationState == CalibrationState.left || calibrationState == CalibrationState.right {
                    GazeCalibrationView(calibrationState: $calibrationState, selectState: $selectState, queue: $queue, value: $value, lookUpSens: $lookUpSens, lookDownSens: $lookDownSens, lookLeftSens: $lookLeftSens, lookRightSens: $lookRightSens, inCalibrationConfirmation: $inCalibrationConfirmation)
                    Spacer()
                }
            } else {
                CalibrationConfirmationView(calibrationState: $calibrationState, selectState: $selectState, queue: $queue, value: $value, lookUpSens: $lookUpSens, lookDownSens: $lookDownSens, lookLeftSens: $lookLeftSens, lookRightSens: $lookRightSens, origLookUp: $origLookUp, origLookDown: $origLookDown, origLookLeft: $origLookLeft, origLookRight: $origLookRight, inCalibrationConfirmation: $inCalibrationConfirmation, playSound: $playSound)
            }
        }
        .onAppear{
            if !setOriginals {
